...
const ContainerRegistriesManagerWidget = () => {
  ...
  const handleDeleteRegistry = (name: string) => async () => {
    await api.deleteContainerRegistry(name)
    fetchRegistries()
  }
          ...
          <div key={registry.name} style={{ flexGrow: 0, flexBasis: '33%', height: '100%' }}>
            <Paper sx={{ p: 2, marginTop: '1rem', marginRight: '1rem', height: '100%' }} elevation={2}>
              ...
              <div>
                ...
                <DeleteButton size={'small'} onConfirm={handleDeleteRegistry(registry.name)}></DeleteButton>
              </div>
            </Paper>
          </div>
        ))}
